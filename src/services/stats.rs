...
impl StatsService {
    ...
    async fn get_daily_stats(&self) -> Result<Vec<DailyStats>, DbErr> {
        ...
        let results = ErrorReport::find()
            ...
            .select_only()
            .column_as(Expr::col(error_report::Column::CreatedAt).cast_as(sea_orm::sea_query::Alias::new("DATE")), "date")
            .column_as(error_report::Column::Id.count(), "count")
            .group_by(Expr::col(error_report::Column::CreatedAt).cast_as(sea_orm::sea_query::Alias::new("DATE")))
            .order_by_asc(Expr::col(error_report::Column::CreatedAt).cast_as(sea_orm::sea_query::Alias::new("DATE")))
            .into_tuple::<(String, i64)>()
            .all(&self.db)
            .await?;
